...
public class CarrelloController {
	...
	//visualizza un determinato carrello
    ...
    @PostMapping 
    public Object createProdotto(@Valid @RequestBody Carrello carrello, HttpServletRequest request, HttpServletResponse response) {
    	...
    	carrello.setUtente(authUser.get());
    	Carrello savedCarrello = carrelloRepository.save(carrello);
        return new ResponseEntity<>(savedCarrello, HttpStatus.CREATED);
    }
    
	@GetMapping("/all")
	public List<Carrello> getAllProdotto () {
	    return carrelloRepository.findAll();
	}
	
	@GetMapping("{id}")
	public ResponseEntity<Carrello> getProdotto (@PathVariable Long id) {
	    Optional<Carrello> OpCarrello = carrelloRepository.findById(id);
	    if (!OpCarrello.isPresent()) {
	        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null);
	    }
	    return ResponseEntity.ok(OpCarrello.get());
	}

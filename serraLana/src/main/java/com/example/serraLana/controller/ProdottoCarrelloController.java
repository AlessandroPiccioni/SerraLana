...
public class ProdottoCarrelloController {
	...
	CarrelloRepository carrelloRepository;
	   ...
	   @PostMapping 
	    public Object createProdottoCarrello(@Valid @RequestBody ProdottoCarrello prodottoCarrello, HttpServletRequest request, HttpServletResponse response) {
	    	...
	    	prodottoCarrello.setCarrello(carrelloRepository.findByUtente_Username(authUser.get().getUsername()));
	    	ProdottoCarrello savedProdottoCarrello = prodottoCarrelloRepository.save(prodottoCarrello);
	        return new ResponseEntity<>(savedProdottoCarrello, HttpStatus.CREATED);
	    }
	    
		@GetMapping("/all")
		public List<ProdottoCarrello> getAllProdottoCarrello () {
		    return prodottoCarrelloRepository.findAll();
		}
		
		@GetMapping("{id}")
		public ResponseEntity<ProdottoCarrello> getProdottoCarrello (@PathVariable Long id) {
		    Optional<ProdottoCarrello> OpProdottoCarrello = prodottoCarrelloRepository.findById(id);
		    if (!OpProdottoCarrello.isPresent()) {
		        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null);
		    }
		    return ResponseEntity.ok(OpProdottoCarrello.get());
		}
	    ...
	    public Object deleteImage(@PathVariable Long id, HttpServletRequest request, HttpServletResponse response) {
	        ...
	        if (!authUser.isPresent() && !authUser.get().getRuolo().equals("admin")) {
	        	...
	        	prodottoCarrelloRepository.deleteById(id);

...
public class ListaDesideriController {
    ...
    @PostMapping 
    public Object createListaDesideri(@Valid @RequestBody ListaDesideri listaDesideri, HttpServletRequest request, HttpServletResponse response) {
    	...
    	listaDesideri.setUtente(authUser.get());
    	ListaDesideri savedListaDesideri = listaDesideriRepository.save(listaDesideri);
        return new ResponseEntity<>(savedListaDesideri, HttpStatus.CREATED);
    }
    
	@GetMapping("/all")
	public List<ListaDesideri> getAllProdotto () {
	    return listaDesideriRepository.findAll();
	}
	
	@GetMapping("{id}")
	public ResponseEntity<ListaDesideri> getProdotto (@PathVariable Long id) {
	    Optional<ListaDesideri> OpListaDesideri = listaDesideriRepository.findById(id);
	    if (!OpListaDesideri.isPresent()) {
	        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null);
	    }
	    return ResponseEntity.ok(OpListaDesideri.get());
	}
    ...
    public Object deleteImage(@PathVariable Long id, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (!authUser.isPresent() && !authUser.get().getRuolo().equals("admin")) {
        	...
        	listaDesideriRepository.deleteById(id);
